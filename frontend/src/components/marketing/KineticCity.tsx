...
const LATEST_MARKET_DATA: MarketItem[] = [
  { label: 'NIFTY 50', price: '23,843', change: 0.86, direction: 'down' },
  { label: 'SENSEX', price: '78,920', change: 0.70, direction: 'down' },
  { label: 'MIDCAP', price: '10,854', change: 1.20, direction: 'down' },
  { label: 'SMALLCAP', price: '8,620', change: 1.40, direction: 'down' },
  { label: 'GOLD', price: '₹7,850/g', change: 0.20, direction: 'up' },
  { label: 'USD/INR', price: '83.50', change: 0.10, direction: 'up', inverse: true }
]
...
function useMarketTicker() {
  ...
  useEffect(() => {
    fetch('/api/market/latest')
      .then(r => r.json())
      .then(d => { if (d.items?.length) setItems(d.items); else setItems(LATEST_MARKET_DATA) })
      .catch(() => setItems(LATEST_MARKET_DATA))
  }, [])
  return items
}
      ...
      {items.map((item, i) => {
        ...
        return (
          <span key={i}>
            ...
            <span style={{ color: itemColor, marginLeft: '4px' }}>
      ...
      >
        ...
        >
          ...
          >
            ...
            <g className="kinetic-anim kc-beam" style={{ transformOrigin: `870px 198px`, animation: 'beam-sweep-spotlight 8s linear infinite' }}>
              <polygon points="870,198 3870,-342 3870,738" fill={C.accent} />
            </g>
